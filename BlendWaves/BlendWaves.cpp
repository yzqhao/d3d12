...
#include "../BlendWaves/FrameResource.h"
...
BlendWaves::BlendWaves(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}
...
void BlendWaves::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void BlendWaves::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void BlendWaves::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void BlendWaves::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void BlendWaves::UpdateCamera(const GameTimer& gt)
{
...
void BlendWaves::UpdateWaves(const GameTimer& gt)
{
...
void BlendWaves::OnKeyboardInput(const GameTimer& gt)
{
...
void BlendWaves::AnimateMaterials(const GameTimer& gt)
{
...
void BlendWaves::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			BlendWavesFR::ObjectConstants objConstants;
...
void BlendWaves::UpdateMainPassCB(const GameTimer& gt)
{
...
void BlendWaves::UpdateMaterialCB(const GameTimer& gt)
{
...
void BlendWaves::Draw(const GameTimer& gt)
{
...
void BlendWaves::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"BlendWaves\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"BlendWaves\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"BlendWaves\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
void BlendWaves::BuildWavesGeometryBuffers()
{
	...
	uint vbByteSize = mWaves->VertexCount() * sizeof(BlendWavesFR::Vertex);
	...
	geo->VertexByteStride = sizeof(BlendWavesFR::Vertex);
...
void BlendWaves::BuildRootSignature()
{
...
void BlendWaves::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void BlendWaves::BuildFrameResources()
{
	for (int i = 0; i < 3; ++i)
	{
		mFrameResources.push_back(std::make_unique<BlendWavesFR::FrameResource>(md3dDevice,
			1, (uint)mAllRitems.size(), (uint)mMaterials.size(), mWaves->VertexCount()));
	}
}

void BlendWaves::LoadTextures()
{
...
void BlendWaves::BuildMaterials()
{
...
void BlendWaves::BuildRenderItems()
{
...
void BlendWaves::BuildLandGeometry()
{
	...
	std::vector<BlendWavesFR::Vertex> vertices(grid.Vertices.size());
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(BlendWavesFR::Vertex);
...
void BlendWaves::BuildBoxGeometry()
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData box = geoGen.CreateBox(8.0f, 8.0f, 8.0f, 3);

	std::vector<BlendWavesFR::Vertex> vertices(box.Vertices.size());
...
void BlendWaves::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(BlendWavesFR::ObjectConstants));
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> BlendWaves::GetStaticSamplers()
{
...
float BlendWaves::GetHillsHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

Math::Vec3 BlendWaves::GetHillsNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	Math::Vec3 n(
