...
#define JYE_AUTOMATIC_FLAGSET(Enum) \
    inline FlagSet<Enum> operator | (const Enum lhs, const Enum rhs) { return FlagSet<Enum>(lhs) | rhs; } \
    inline FlagSet<Enum> operator & (const Enum lhs, const Enum rhs) { return FlagSet<Enum>(lhs) & rhs; } \
    inline FlagSet<Enum> operator ^ (const Enum lhs, const Enum rhs) { return FlagSet<Enum>(lhs) ^ rhs; } \
    inline FlagSet<Enum> operator ~ (const Enum rhs) { return ~FlagSet<Enum>(rhs); }

/// Declare FlagSet for specific enum and create operators for automatic FlagSet construction.
#define JYE_FLAGSET(enumName, flagsetName) \
    JYE_AUTOMATIC_FLAGSET(enumName); \
