...
void LitColumns::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			LitColumnsFR::ObjectConstants objConstants;
...
void LitColumns::UpdateMaterialCB(const GameTimer& gt)
{
...
void LitColumns::BuildFrameResources()
{
	...
	{
		mFrameResources.push_back(std::make_unique<LitColumnsFR::FrameResource>(md3dDevice,
...
void LitColumns::BuildMaterials()
{
	...
	bricks0->DiffuseAlbedo.set(0.133333340f, 0.545098066f, 0.133333340f, 1.0f);
	...
	stone0->DiffuseAlbedo.set(0.690196097f, 0.768627524f, 0.870588303f, 1.0f);
	...
	tile0->DiffuseAlbedo.set(0.827451050f, 0.827451050f, 0.827451050f, 1.0f);
	...
	skullMat->FresnelR0.set(0.05f, 0.05f, 0.05);
...
void LitColumns::BuildShapeGeometry()
{
	...
	std::vector<LitColumnsFR::Vertex> vertices(totalVertexCount);
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(LitColumnsFR::Vertex);
	...
	geo->VertexByteStride = sizeof(LitColumnsFR::Vertex);
...
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(LitColumnsFR::ObjectConstants));
