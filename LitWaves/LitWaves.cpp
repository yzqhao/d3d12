...
void LitWaves::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			LitWavesFR::ObjectConstants objConstants;
...
void LitWaves::UpdateMainPassCB(const GameTimer& gt)
{
	...
	Math::Vec4 lightDir = -SphericalToCartesian(1.0f, mSunTheta, mSunPhi);
	mMainPassCB.Lights[0].Direction.set(lightDir.x, lightDir.y, lightDir.z);
...
void LitWaves::UpdateMaterialCB(const GameTimer& gt)
{
...
void LitWaves::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"LitWaves\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"LitWaves\\Default.hlsl", nullptr, "PS", "ps_5_0");
...
void LitWaves::BuildWavesGeometryBuffers()
{
	...
	uint vbByteSize = mWaves->VertexCount() * sizeof(LitWavesFR::Vertex);
	...
	geo->VertexByteStride = sizeof(LitWavesFR::Vertex);
...
void LitWaves::BuildFrameResources()
{
	for (int i = 0; i < 3; ++i)
	{
		mFrameResources.push_back(std::make_unique<LitWavesFR::FrameResource>(md3dDevice,
...
void LitWaves::BuildMaterials()
{
	...
	grass->Name = "grass";
	grass->MatCBIndex = 0;
	grass->DiffuseAlbedo.set(0.2f, 0.6f, 0.2f, 1.0f);
	...
	water->Name = "water";
	water->MatCBIndex = 1;
	water->DiffuseAlbedo.set(0.0f, 0.2f, 0.6f, 1.0f);
...
void LitWaves::BuildLandGeometry()
{
	...
	std::vector<LitWavesFR::Vertex> vertices(grid.Vertices.size());
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(LitWavesFR::Vertex);
...
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(LitWavesFR::ObjectConstants));
...
Math::Vec3 LitWaves::GetHillsNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	Math::Vec3 n(
