...
#include "NormalMap.h"
#include "../NormalMap/FrameResource.h"
...
void NormalMap::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
...
void NormalMap::OnKeyboardInput(const GameTimer& gt)
{
...
void NormalMap::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			NormalMapFR::ObjectConstants objConstants;
...
void NormalMap::UpdateMainPassCB(const GameTimer& gt)
{
...
void NormalMap::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		{
			NormalMapFR::MaterialData matData;
...
void NormalMap::Draw(const GameTimer& gt)
{
...
void NormalMap::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"NormalMap\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"NormalMap\\Default.hlsl", nullptr, "PS", "ps_5_1");

	mShaders["skyVS"] = d3dUtil::CompileShader(L"NormalMap\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L"NormalMap\\Sky.hlsl", nullptr, "PS", "ps_5_1");
...
void NormalMap::BuildDescriptorHeaps()
{
	...
	auto skyCubeMap = mTextures["skyNormalMap"]->Resource;
...
void NormalMap::BuildFrameResources()
{
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.push_back(std::make_unique<NormalMapFR::FrameResource>(md3dDevice,
...
void NormalMap::BuildRenderItems()
{
...
void NormalMap::BuildShapeGeometry()
{
	...
	std::vector<NormalMapFR::Vertex> vertices(totalVertexCount);
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(NormalMapFR::Vertex);
	...
	geo->VertexByteStride = sizeof(NormalMapFR::Vertex);
...
void NormalMap::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(NormalMapFR::ObjectConstants));
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> NormalMap::GetStaticSamplers()
{
