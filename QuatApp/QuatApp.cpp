...
#include "QuatApp.h"
#include "../QuatApp/FrameResource.h"
...
void QuatApp::DefineSkullAnimation()
{
	...
	mSkullAnimation.Keyframes[0].Translation = { -7.0f, 0.0f, 0.0f };
	mSkullAnimation.Keyframes[0].Scale = { 0.25f, 0.25f, 0.25f };
	mSkullAnimation.Keyframes[0].RotationQuat.FromAngleAxis(MATH_RAD_TO_DEG(-30), Math::Vec3(0.0f, 1.0f, 0.0f));

	mSkullAnimation.Keyframes[1].TimePos = 2.0f;
	mSkullAnimation.Keyframes[1].Translation = { 0.0f, 2.0f, 10.0f };
	mSkullAnimation.Keyframes[1].Scale = { 0.5f, 0.5f, 0.5f };
	mSkullAnimation.Keyframes[1].RotationQuat.FromAngleAxis(MATH_RAD_TO_DEG(-45), Math::Vec3(1.0f, 1.0f, 2.0f));

	mSkullAnimation.Keyframes[2].TimePos = 4.0f;
	mSkullAnimation.Keyframes[2].Translation = { 7.0f, 0.0f, 0.0f };
	mSkullAnimation.Keyframes[2].Scale = { 0.25f, 0.25f, 0.25f };
	mSkullAnimation.Keyframes[2].RotationQuat.FromAngleAxis(MATH_RAD_TO_DEG(30), Math::Vec3(0.0f, 1.0f, 0.0f));

	mSkullAnimation.Keyframes[3].TimePos = 6.0f;
	mSkullAnimation.Keyframes[3].Translation = { 0.0f, 1.0f, -10.0f };
	mSkullAnimation.Keyframes[3].Scale = { 0.5f, 0.5f, 0.5f };
	mSkullAnimation.Keyframes[3].RotationQuat.FromAngleAxis(MATH_RAD_TO_DEG(-70), Math::Vec3(1.0f, 0.0f, 0.0f));

	mSkullAnimation.Keyframes[4].TimePos = 8.0f;
	mSkullAnimation.Keyframes[4].Translation = { -7.0f, 0.0f, 0.0f };
	mSkullAnimation.Keyframes[4].Scale = { 0.25f, 0.25f, 0.25f };
	mSkullAnimation.Keyframes[4].RotationQuat.FromAngleAxis(MATH_RAD_TO_DEG(-30), Math::Vec3(0.0f, 1.0f, 0.0f));
}
...
void QuatApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			QuatAppFR::ObjectConstants objConstants;
...
void QuatApp::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		{
			QuatAppFR::MaterialData matData;
...
void QuatApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"QuatApp\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"QuatApp\\Default.hlsl", nullptr, "PS", "ps_5_1");

	mShaders["skyVS"] = d3dUtil::CompileShader(L"QuatApp\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L"QuatApp\\Sky.hlsl", nullptr, "PS", "ps_5_1");
...
void QuatApp::BuildDescriptorHeaps()
{
	...
	auto skyTex = mTextures["skyQuatApp"]->Resource;
...
void QuatApp::BuildFrameResources()
{
	...
	{
		mFrameResources.push_back(std::make_unique<QuatAppFR::FrameResource>(md3dDevice,
...
void QuatApp::BuildShapeGeometry()
{
	...
	std::vector<QuatAppFR::Vertex> vertices(totalVertexCount);
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(QuatAppFR::Vertex);
	...
	geo->VertexByteStride = sizeof(QuatAppFR::Vertex);
...
void QuatApp::BuildSkullGeometry()
{
	...
	std::vector<QuatAppFR::Vertex> vertices(vcount);
...
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(QuatAppFR::ObjectConstants));
