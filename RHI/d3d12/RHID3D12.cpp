...
RHID3D12* RHID3D12::Instance()
{
	return s_rhi;
}

bool RHID3D12::Init(HWND window)
{
...
void RHID3D12::CreateRtvAndDsvDescriptorHeaps()
{
...
void RHID3D12::OnResize(int w, int h)
{
	assert(md3dDevice);
	assert(mSwapChain);
...
void RHID3D12::BeginRender()
{
	...
	mCommandList->ClearRenderTargetView(CurrentBackBufferView(), Math::Color::GRAY.getPtr(), 0, nullptr);
...
ID3D12Resource* RHID3D12::CurrentBackBuffer()const
{
	return mSwapChainBuffer[mCurrBackBuffer];
}

D3D12_CPU_DESCRIPTOR_HANDLE RHID3D12::CurrentBackBufferView()const
{
...
D3D12_CPU_DESCRIPTOR_HANDLE RHID3D12::DepthStencilView()const
{
	return mDsvHeap->GetCPUDescriptorHandleForHeapStart();
}

bool RHID3D12::Get4xMsaaState()const
{
	return m4xMsaaState;
}

void RHID3D12::Set4xMsaaState(bool value)
{
...
void RHID3D12::LogAdapterOutputs(IDXGIAdapter* adapter)
{
	UINT i = 0;
...
void RHID3D12::LogOutputDisplayModes(IDXGIOutput* output, DXGI_FORMAT format)
{
	UINT count = 0;
	UINT flags = 0;
...
bool RHID3D12::OnLoadVShaderProgram(RHIVSShader* pVShaderProgram, RHIResourceIdentifier*& pID)
{
	RHIShaderID* pVShaderID = NULL;
	pVShaderID = new RHIShaderID;
	if (!pVShaderID)
		return 0;
	pID = pVShaderID;
	...
	{
		CreateVShaderFromString(pVShaderProgram, pVShaderID);
	}
	else
	{
		CreateVShaderFromCache(pVShaderProgram, pVShaderID);
	}
...
bool RHID3D12::OnLoadPShaderProgram(RHIPSShader* pPShaderProgram, RHIResourceIdentifier*& pID)
{
	RHIShaderID* pPShaderID = NULL;
	pPShaderID = new RHIShaderID;
	if (!pPShaderID)
		return 0;
	pID = pPShaderID;
	...
	{
		CreatePShaderFromString(pPShaderProgram, pPShaderID);
	}
	else
	{
		CreatePShaderFromCache(pPShaderProgram, pPShaderID);
	}
