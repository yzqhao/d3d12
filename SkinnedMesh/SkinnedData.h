...
struct AnimationClip
{
    ...
    void Interpolate(float t, std::vector<Math::Mat4>& boneTransforms)const;
...
public:
	...
	void Set(
	 ...
	 // the same timePos.
    void GetFinalTransforms(const std::string& clipName, float timePos, std::vector<Math::Mat4>& finalTransforms)const;

private:
    // Gives parentIndex of ith bone.
	std::vector<int> mBoneHierarchy;

	std::vector<Math::Mat4> mBoneOffsets;
