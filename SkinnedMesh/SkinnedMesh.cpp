...
void SkinnedMesh::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void SkinnedMesh::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void SkinnedMesh::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void SkinnedMesh::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
...
void SkinnedMesh::LoadSkinnedModel()
{
...
void SkinnedMesh::OnKeyboardInput(const GameTimer& gt)
{
...
void SkinnedMesh::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		{
			SkinnedMeshFR::ObjectConstants objConstants;
			objConstants.World = e->World;
			objConstants.TexTransform = e->TexTransform;
			objConstants.MaterialIndex = e->Mat->MatCBIndex;
			objConstants.NormalMapIndex = e->Mat->NormalSrvHeapIndex;
...
void SkinnedMesh::UpdateMainPassCB(const GameTimer& gt)
{
...
void SkinnedMesh::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		{
			SkinnedMeshFR::MaterialData matData;
...
void SkinnedMesh::UpdateSkinnedCBs(const GameTimer& gt)
{
	...
	SkinnedMeshFR::SkinnedConstants skinnedConstants;
	std::copy(
...
void SkinnedMesh::Draw(const GameTimer& gt)
{
...
void SkinnedMesh::BuildShadersAndInputLayout()
{
	HRESULT hr = S_OK;
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"SkinnedMesh\\Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"SkinnedMesh\\Default.hlsl", nullptr, "PS", "ps_5_1");

	mShaders["skyVS"] = d3dUtil::CompileShader(L"SkinnedMesh\\Sky.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["skyPS"] = d3dUtil::CompileShader(L"SkinnedMesh\\Sky.hlsl", nullptr, "PS", "ps_5_1");
...
void SkinnedMesh::BuildRootSignature()
{
...
void SkinnedMesh::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void SkinnedMesh::BuildFrameResources()
{
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.push_back(std::make_unique<SkinnedMeshFR::FrameResource>(md3dDevice,
			1, (uint)mAllRitems.size(), 1, (uint)mMaterials.size()));
	}
}

void SkinnedMesh::BuildMaterials()
{
...
void SkinnedMesh::BuildRenderItems()
{
	...
	for (UINT i = 0; i < mSkinnedMats.size(); ++i)
	{
		...
		Math::Mat4 modelScale;
		modelScale.scale(0.05f, 0.05f, -0.05f);
		Math::Mat4 modelRot;
...
void SkinnedMesh::BuildShapeGeometry()
{
	...
	std::vector<SkinnedMeshFR::Vertex> vertices(totalVertexCount);
	...
	const uint vbByteSize = (uint)vertices.size() * sizeof(SkinnedMeshFR::Vertex);
	const uint ibByteSize = (uint)indices.size() * sizeof(std::uint16_t);
	...
	geo->VertexByteStride = sizeof(SkinnedMeshFR::Vertex);
	geo->VertexBufferByteSize = vbByteSize;
	geo->IndexFormat = DXGI_FORMAT_R16_UINT;
	geo->IndexBufferByteSize = ibByteSize;
...
void SkinnedMesh::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	uint objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(SkinnedMeshFR::ObjectConstants));
	UINT skinnedCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(SkinnedMeshFR::SkinnedConstants));
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> SkinnedMesh::GetStaticSamplers()
{
