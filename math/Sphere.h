...
inline void Sphere::set(const Vec3& p0, const Vec3& p1)
{
	...
	_center = dhalf + p0;
	_radius = length(dhalf);
}

inline bool Sphere::IsInside(const Sphere& inSphere)const
{
	float sqrDist = lengthSquared(GetCenter() - inSphere.GetCenter());
...
{
	float sqrDist = lengthSquared(inSphere0.GetCenter() - inSphere1.GetCenter());
...
{
	return Max(0.0f, lengthSquared(p - s.GetCenter()) - SQUARE(s.GetRadius()));
}
